...
def ideal_diamond(line_img, formatted_lines):
    ...
    for corner in corners:
        cv.circle(corner_img, corner, 8, (0, 0, 255), -1)
    ...
    colour = (255, 0, 0)
    cv.line(diamond_img, tuple(np.array(corners[0]).astype(int)), tuple(np.array(corners[1]).astype(int)), colour, 1) # top to right
    cv.line(diamond_img, tuple(np.array(corners[1]).astype(int)), tuple(np.array(corners[2]).astype(int)), colour, 1) # right to bottom
    cv.line(diamond_img, tuple(np.array(corners[2]).astype(int)), tuple(np.array(corners[3]).astype(int)), colour, 1) # bottom to left
    cv.line(diamond_img, tuple(np.array(corners[3]).astype(int)), tuple(np.array(corners[0]).astype(int)), colour, 1) # left to top
    cv.line(diamond_img, tuple(np.array(corners[0]).astype(int)), tuple(np.array(corners[2]).astype(int)), colour, 1) # top to bottom
...
def post_processing(corners, valid_lines):
    ...
    CopyDiamond_A = arrangement_evaluation(corners)
...
def process_image(file_path):
    ...
    CopyDiamond_S, CopyDiamond_D, CopyDiamond_A, CopyDiamond, CopyDiamond_SV = post_processing(corners, valid_lines)
    return CopyDiamond_S, CopyDiamond_D, CopyDiamond_A, CopyDiamond, CopyDiamond_SV